	...
	<div class="all_objectives_content">
		<ul>
				<li class="box_content">
					<h4>
						<?php if ($objectives['already_follow'] == 0) : ?>
						...
						<?php endif ?>
						<div class="id_objective form_inline" type="hidden" value="<?php echo $objectives['id'];?>"></div>
						<?php echo UrlToShortLink(htmlspecialchars(stripslashes($objectives['name_obj']))); ?>
					</h4>
					...
					<div class="hidden_part">
						<div>
							<h6> 
								Par : <a href="/user/profile/<?php echo $objectives['id_membres']?>"><?php echo $objectives['firstname']." ".$objectives['lastname']; ?></a>
								 | <?php if ($objectives['done'] == 0 ){ echo "Objectif en cours";} else { echo "Objectif terminé";}?>
							</h6>
						</div>
						<?php //Steps ?>
						<div class="box_content_steps">
							<ul>
								...
								<?php foreach ($steps_objectives as $value) : ?>
									<?php if ($value['id_objective'] == $objectives['id']) : ?>
									...
									<?php if ($value['id_objective'] == $objectives['id']) : ?>
										<li class="box_advice">
											...
											<div class="form_inline form_modif_advices">
												...
												<?php endif ?>
												<?php if ($objectives['id_membres'] == $user_id OR $value['id_member_give_advice'] == $user_id) : ?>
						...
						<?php if ($objectives['done'] == 0) : ?>
							<?php //Add advice ?>
							<div class="add_advice_form">
								<div class="success_add_advice"></div>
								<div class="id_objective" type="hidden" value="<?php echo $objectives['id'];?>"></div>
						...
						<?php endif ?>
						<h6>Catégorie : <?php echo $objectives['category'] ?> | Ajouté le : <?php echo $objectives['date_creation'] ?></h6>
					</div>
				</li>
		</ul>
	</div>
